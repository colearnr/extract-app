...
class Extract extends CoreApp {
  ...
  quickProcess (user, lbit, callback) {
    ...
    if (url) {
      this.processUrl(url, true, function (err, elib, html) {
        ...
        _.merge(meta, analyser.getBody(html))
        meta.img_url = [meta.image]
        callback(err, meta)
      })
    } else {
      callback(null, {})
  ...
  processLearnbit (user, lbit, callback) {
    ...
    if (url) {
      ...
      return this.processHtml(lbit.body, false, function (err, meta, html) {
        self.notifyExtractResult(user, lbit._id, meta, html)
        callback(err, meta)
      })
    } else {
  ...
  processUrl (url, lazy, callback) {
    if (url) {
      ...
      }, (error, response, body) => this.processHtml(body, lazy, callback))
    } else {
      callback(null, null)
    }
  }
   ...
   * @param callback Callback
   */
  processHtml (html, lazy, callback) {
    if (lazy) {
      ...
      meta.img_url = meta.image ? [meta.image] : []
      async.parallel({
        keywords: (cb) => nlpAnalyser.keywords(html, cb),
        purifyText: (cb) => nlpAnalyser.purifyText(html, cb),
        sentiment: (cb) => nlpAnalyser.sentiment(html, cb)
      }, (err, results) => {
