...
class Extract extends CoreApp {
  ...
  quickProcess(user, lbit, callback) {
    ...
    let self = this
    if (url) {
      this.processUrl(url, true, function (err, elib) {
        let meta = {}
  ...
  processLearnbit(user, lbit, callback) {
    ...
    let self = this
    if (url) {
      return this.processUrl(url, false, function (err, meta) {
        self.notifyExtractResult(user, lbit._id, meta)
        callback(err, meta)
      })
    } else if (lbit.body) {
      return this.processHtml(lbit.body, false, function (err, meta) {
        self.notifyExtractResult(user, lbit._id, meta)
        callback(err, meta)
      })
    } else {
  ...
  processHtml(html, lazy, callback) {
    if (lazy) {
      callback(null, extractor.lazy(html || ''))
    } else {
