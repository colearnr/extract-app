...
class Analyse {
  ...
  getBody (html) {
    ...
    let mainText = this.strip_tags(body[0], self.dom_separator)
    let wordsCount = mainText.split(' ').length
    let time = (wordsCount / 200) * 60
    return {body: body[0], text: mainText, wordsCount, time, friendlyTime: this.friendlyTime(time)}
  }
  ...
  strip_tags (html, separator) {
    ...
    let self = this
    let st = html.replace(/<.+?>/mg, separator)
