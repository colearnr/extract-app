  ...
  getBody (html, bodyContent) {
    ...
    if (!bodyContent || bodyContent === '') {
      ...
      list.forEach(function (block) {
        ...
        block = sanitizeHtml(block, {
  allowedTags: sanitizeHtml.defaults.allowedTags.concat([ 'img', 'main', 'article', 'section' ])
})
        if (body.length > 0) {
    ...
    return {body: bodyContent, bodyList: body, text: mainText, wordsCount, time, friendlyTime: this.friendlyTime(time)}
  }
